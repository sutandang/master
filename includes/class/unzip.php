...
// 28/11/2005 (2.4)
// - unzip is now compliant with old-style "Data Description", made by some compressors,
...
#
#  Methods:
#  * unzip($filename)         - Constructor - Opens $filename
...
class Unzip
{
	...
	function getList($stopOnFile=false){
		...
		if($this->debug){
			...
			if(sizeof($this->endOfCentral)){
				...
				echo "<tr style='background: #DAA'><td colspan='2'>unzip - End of file</td></tr>";
	...
	function unzip2($compressedFileName, $targetFileName=false, $applyChmod=0777){
		if(!sizeof($this->compressedList)){
		...
		$a   = fread($this->fh, $fdetails['compressed_size']);
		$ret = $this->uncompress(
				$a,
	...
	{
		if($targetDir === false)
			$targetDir = dirname($_SERVER['SCRIPT_FILENAME']);//$baseDir;
		...
		foreach($lista as $fileName=>$trash)
		{
			...
			$maintainStructure?
				$this->unzip2($fileName, "$targetDir/$fileName", $applyChmod):
				$this->unzip2($fileName, "$targetDir/".basename($fileName), $applyChmod);
		}
	}
	...
	function debugMsg($level, $string){
		if($this->debug){
			if($level == 1)
				echo "<b style='color: #777'>unzip:</b> $string<br>";

			if($level == 2)
				echo "<b style='color: #F00'>unzip:</b> $string<br>";
		}
