<?php
	...
	class Search
	{
		...
		function changeKeywordPatern($key, $value)
		{
			if ($key=='name')
			{
				...
				while(false !== ($file = readdir($dirHandler)))
				{
					if($file != '.' && $file != '..')
					{
						...
						if(is_file($path))
						{
							...
							if($this->searchkeywords['name'] !== ''  && preg_match($this->changeKeywordPatern('name', $this->searchkeywords['name']), $file))
							{
								$isValid = true;
							}else
							...
							if($this->searchkeywords['size_from'] != '' && $fileSize < $this->searchkeywords['size_from'])
							{
								$isValid = true;
							}else
							if($this->searchkeywords['size_to'] != '' && $fileSize > $this->searchkeywords['size_to'])
							{
								$isValid = true;
							}
...
/*								$tem['size'] = transformFileSize($tem['size']);
						...
						if(is_dir($path) && $this->searchkeywords['recursive'])
						{
