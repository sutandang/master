...
$userID = @intval($_SESSION['bbcAuthAdmin']['id']);
$denied = ($userID) ? false : true;
$path   = '';
if (!$denied)
{
	if (!empty($_GET['filemanagerPath'])
		&& preg_match('~^images/~s', $_GET['filemanagerPath'])
		&& is_dir(_ROOT.$_GET['filemanagerPath']))
	{
		$path = preg_replace('~^'.preg_quote(_ROOT, '~').'~', '', realpath(_ROOT.$_GET['filemanagerPath'])).'/';
		$_SESSION['filemanagerPath'] = $path;
	}else
	if (!empty($_SESSION['filemanagerPath']) && !isset($_GET['filemanagerPath']))
	{
		$path = $_SESSION['filemanagerPath'];
	}
	if (empty($path) || (!empty($path) && !file_exists(_ROOT.$path)))
	{
		$path = 'images/uploads/';
	}
}
 ...
 * by session => true
 */
define('CONFIG_ACCESS_CONTROL_MODE', $denied);
define("CONFIG_LOGIN_USERNAME", md5(rand()));
define('CONFIG_LOGIN_PASSWORD', md5(rand()));
...
/*
	...
	and it must be a folder under the CONFIG_SYS_DEFAULT_PATH or the same folder
	these two paths accept relative path only, don't use absolute path
*/
define('CONFIG_SYS_DEFAULT_URL', _URL.$path);
define('CONFIG_SYS_DEFAULT_PATH', _ROOT.$path);
define('CONFIG_SYS_FOLDER_SHOWN_ON_TOP', true); //show your folders on the top of list if true or order by name
define("CONFIG_SYS_DIR_SESSION_PATH", _CACHE.'filemanager/');
...
$r = array('path', 'search_folder', 'current_folder_path', 'folder');
foreach ($r as $k)
{
	if (isset($_GET[$k]))
	{
		if (empty($_GET[$k]))
		{
			$_GET[$k] = CONFIG_SYS_DEFAULT_PATH;
		}else{
			$_GET[$k] = CONFIG_SYS_DEFAULT_PATH.preg_replace(array('~^'.preg_quote(CONFIG_SYS_DEFAULT_PATH, '~').'~s', '~^/~s'), array('',''), $_GET[$k]);
		}
	}
}
...
define('CONFIG_SYS_EXC_FILE_PATTERN', 'index.html'); //will prevent listing of files with such pattern(s). separated by , if multiple
...
define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,bmp,swf'); //
...
define('CONFIG_UPLOAD_INVALID_EXTS', 'php,cgi,phtml');
...
define('CONFIG_WEBSITE_DOCUMENT_ROOT', _ROOT);
...
define('CONFIG_DEFAULT_PAGINATION_LIMIT', 20);
