...
function transformFilePath($value) {
	$rootPath = addTrailingSlash(backslashToSlash(getRealPath(CONFIG_SYS_DEFAULT_PATH)));
...
function addNoCacheHeaders() {
	...
	{
		$roorPath = strtolower(addTrailingSlash(backslashToSlash(getRealPath(CONFIG_SYS_DEFAULT_PATH))));
...
function myRealPath($path) {
 ...
 function getRealPath($value)
 {
	...
	{
		return CONFIG_SYS_DEFAULT_URL.preg_replace(array('~^'.preg_quote(CONFIG_SYS_DEFAULT_PATH, '~').'~s', '~^/~s'), array('',''), $value);
		$output = '';
		$wwwroot = removeTrailingSlash(backslashToSlash(getRootPath()));
...
function getRootPath() {
     ...
     */
        function isInvalidPattern( $pattern, $string)
        {
            ...
            if (strpos($pattern,",")!==false)
            {
                ...
                foreach ($regExps as $regExp => $value)
                {
									$value1 = trim($value, 'ise');
									if (substr($value1, 0, 1)!=substr($value1, -1))
									{
										$value = '~'.preg_quote($value, '~').'~is';
									}
                  if(preg_match($value, $string))
                  {
                      return true;
							...
							$pattern1 = trim($pattern, 'ise');
							if (substr($pattern1, 0, 1)!=substr($pattern1, -1))
							{
  ...
  function getParentFolderPath($path)
  {
  	...
  	if(isUnderRoot($parentPath))
  	{
  		...
  		return str_replace(CONFIG_SYS_DEFAULT_PATH, '', $parentPath);
  	}else{
  		return '';
  	}
  }
