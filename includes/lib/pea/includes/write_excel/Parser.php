...
*/
  function _match($token)
    ...
    switch($token)
        ...
        default:
	    // if it's a reference
            if(preg_match("~^[A-I]?[A-Z][0-9]+$~is",$token) and
	       !preg_match("~[0-9]~s",$this->_lookahead) and
               ($this->_lookahead != ':') and ($this->_lookahead != '.'))
                {
                return($token);
                }
            // if it's a range (A1:A2)
            elseif(preg_match("~^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$~is",$token) and
	           !preg_match("~[0-9]~s",$this->_lookahead))
	        {
		return($token);
		}
            // if it's a range (A1..A2)
		...
		return($token);
            ...
            elseif(preg_match("~^[A-Z0-9À-Ü\.]+$~is",$token) and ($this->_lookahead == "("))
...
*/
  function _fact()
    ...
    // if it's a reference
    if (preg_match("~^[A-I]?[A-Z][0-9]+$~is",$this->_current_token))
        {
    ...
    // if it's a range
    elseif (preg_match("~^[A-I]?[A-Z][0-9]+:[A-I]?[A-Z][0-9]+$~is",$this->_current_token) or
            preg_match("~^[A-I]?[A-Z][0-9]+\.\.[A-I]?[A-Z][0-9]+$~is",$this->_current_token))
        {
    ...
    // if it's a function call
    elseif (preg_match("~^[A-Z0-9À-Ü\.]+$~is",$this->_current_token))
        {
        $result = $this->_func();
        return($result);
        }
